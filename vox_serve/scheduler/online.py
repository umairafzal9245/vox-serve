    ...
    def _select_lm_requests(self) -> List[Request]:
        ...
        if prefill_requests:
            ...
            for req in prefill_requests:
                ...
                # Batch as many prefill requests as fit in the prefill CUDA graph.
                # Always admit at least one request (current_batch_size == 0) so a
                # single long prompt never stalls; batch additional prefills only
                # while both the batch-size and seq-len budgets allow it. The extra
                # ``max_prefill_batch_size`` margin accounts for graph batch padding.
                if current_batch_size == 0 or (
                    current_batch_size + 1 <= max_prefill_batch_size
                    and current_seq_len + req_seq_len + max_prefill_batch_size <= max_seq_len
                ):
                    ...
                    # Batch or seq-len budget reached; remaining prefills run next step.
                    break
