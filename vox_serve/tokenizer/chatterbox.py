...
class ChatterboxDecoder(nn.Module):
    ...
    def __init__(self):
        ...
        # Number of CFM (flow-matching) Euler steps used by the token->mel decoder.
        # Fewer steps => lower detokenization latency at some quality cost. This is
        # the dominant term in Time-To-First-Audio for concurrent streaming.
        self.n_timesteps = 10
    ...
    def decode(
        ...
        # Run the flow-matching decoder (the TTFA-dominant stage) under bf16
        # autocast. The sibling CosyVoice2 detokenizer already runs this exact
        # module in bf16, so this matches a validated configuration while roughly
        # halving latency. The HiFT vocoder below stays in fp32 (mels are cast
        # back to float) to preserve waveform quality.
        ...
        output_mels = output_mels[:, :, ref_dict["prompt_feat_len"] :].float()
