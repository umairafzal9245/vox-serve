...
from ..tokenizer.qwen3_codec import Qwen3TTSDecoder, Qwen3TTSDecoderCache
...
class Qwen3TTSModel(BaseLMWithDepth):
    ...
    def output_audio_length(self) -> int:
        ...
        return self.detokenize_interval * 1920
    ...
    def audio_decoder_initial_cache(self, batch_size: int) -> Optional[Qwen3TTSDecoderCache]:
        ...
        Returns:
            Qwen3TTSDecoderCache initialized with zeros for the given batch size.
            Includes pre-allocated work buffers and output buffers for CUDA graph compatibility.
        """
    ...
    def sampling(
        ...
        **kwargs,
        ...
        # Separate check for max_tokens - must run for all requests regardless of stop token
        for req in requests:
    ...
    def postprocess(
        ...
        **kwargs,
        ...
        Args:
            ...
            decoder_cache: Optional cache for streaming inference
            **kwargs: Additional model-specific parameters
        ...
        if decoder_cache is not None:
            ...
            audio_tensor, new_cache = self.audio_decoder.decode_chunk(
                tokens_to_process,
                decoder_cache=decoder_cache,
            )
            # In-place update for CUDA graph compatibility
            decoder_cache.copy_from(new_cache)
        else:
            # Non-streaming mode (full decode)
