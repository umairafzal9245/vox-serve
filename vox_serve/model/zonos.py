...
class ZonosModel(BaseLM):
    def __init__(
        ...
        # Default 40 trades a little TTFA for cleaner audio: larger DAC chunks
        # reduce chunk-boundary artifacts and amplitude wobble vs 30, while still
        # being well below the original 50. (25 clearly degrades.) Override with
        # VOX_ZONOS_DETOK_INTERVAL. Must stay > n_codebooks.
        self._detokenize_interval = max(
            self._n_codebooks + 1, int(os.environ.get("VOX_ZONOS_DETOK_INTERVAL", "40"))
        )
        ...
        # Conditioning (speaking_rate / pitch_std / emotion) is left to Zonos's
        # own defaults unless a request overrides it per-call. See preprocess().

        self._get_default_speaker_embedding()

        # Speaker-embedding cache for voice cloning. Extracting an embedding from a
        # reference (load + resample + ResNet293 speaker encoder) is ~20 ms and runs
        # serialized in preprocess, so it adds N x 20 ms to TTFA for N concurrent
        # clone requests. Cache by reference *content* hash: repeated references (the
        # common per-speaker case) reuse the exact same embedding and skip re-encoding.
        self._spk_emb_cache: "OrderedDict[str, torch.Tensor]" = OrderedDict()
        self._spk_emb_cache_max = 256
    ...
    def supports_audio_input(self) -> bool:
        """Zonos accepts reference audio for zero-shot voice cloning."""
        return True
    ...
    def _speaker_embedding_from_audio(self, audio_path: str) -> torch.Tensor:
        """Extract a speaker embedding from reference audio for voice cloning.

        The speaker encoder resamples to 16 kHz and downmixes to mono internally,
        so any sample rate / channel layout that torchaudio can load is accepted.
        Returns a [1, embedding_dim] tensor matching the default speaker embedding.

        Results are cached by reference-audio content hash so that repeated use of
        the same reference (the common case) skips the ~20 ms load+encode entirely.
        The cached tensor is bit-identical to a fresh computation, so audio output
        is unchanged.
        """
        ...
        cached = self._spk_emb_cache.get(key)
        if cached is not None:
            self._spk_emb_cache.move_to_end(key)
            return cached

        wav, sr = torchaudio.load(audio_path)
        _, spk_embedding = self.speaker_encoder(wav.to(self.device), sr)
        emb = spk_embedding.unsqueeze(0).bfloat16()

        self._spk_emb_cache[key] = emb
        if len(self._spk_emb_cache) > self._spk_emb_cache_max:
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        # Only pass conditioning that the request explicitly sets; anything left
        # unset falls through to Zonos's own built-in defaults in _make_cond_dict
        # (speaking_rate=15, pitch_std=20, neutral-leaning emotion). Custom global
        # defaults are intentionally not applied here because aggressive pitch_std
        # / emotion values destabilize alignment and drop words.
        cond_kwargs = {}
