...
class ChatterboxModel(BaseLM):
    def __init__(
        ...
        # Enable TF32 for the detokenizer's conv/matmul kernels. This is a
        # numerically safe speedup on Ampere+ GPUs and materially reduces the
        # S3Gen flow/vocoder latency that dominates streaming TTFA.
        torch.backends.cuda.matmul.allow_tf32 = True
        torch.backends.cudnn.allow_tf32 = True

        # CFM diffusion steps for the token->mel decoder. Upstream uses 10; this
        # stage dominates streaming Time-To-First-Audio under concurrency. 6 steps
        # is a strong quality/latency balance (~600 ms 32-stream TTFA, 100%
        # streaming viability); drop to 4 for <500 ms or raise to 10 for max
        # fidelity. Overridable via VOX_CHATTERBOX_CFM_STEPS.
        import os
        self.audio_decoder.n_timesteps = int(os.environ.get("VOX_CHATTERBOX_CFM_STEPS", "6"))
